...
### Added
- **Coordinator role (FEAT-01):** new `coordinator` role with a "Mi Cartera" view that shows and approves only the clients in that coordinator's portfolio (RACI PM mapping). New `coordinator_assignments` table seeded from the glossary; MCP tools `get_my_scope`, `get_coordinator_overview`, and scope-filtered `get_pending_time_records`.

### Fixed
- **CORS:** allow Cloudflare Pages preview subdomains of our projects (prevents login CORS failures on preview URLs).

### Notes
- Migration 0018 sets role `coordinator` for the employee-PMs (Federico Cristofani, Lucía Manera, Pedro Lizondo, Augusto Morelli) and seeds their portfolios. Full suite green (106 tests); build OK.
...
### Fixed
- **Fix ReferenceError en Dashboard (`EmailRemindersModal`)**:
  - Restablecida la importación de `EmailRemindersModal` en `Dashboard.tsx` resolviendo el error `ReferenceError: EmailRemindersModal is not defined`.
